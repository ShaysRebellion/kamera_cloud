...
if ! grep -q mqtt.kamera-cloud-rabbitmq.io /etc/hosts; then
    sudo echo -e "# For connecting to rabbitmq broker within (multipass) virtual machine (minikube) cluster" >> /etc/hosts
    sudo echo -e "192.168.205.2 mqtt.kamera-cloud-rabbitmq.io\n" >> /etc/hosts
fi
